...
public class FactoryExample {
  ...
  public void getResult(Factory mainFactory, char mainFactorySkill, IWorkingTogether partner) {
    ...
    if (partner instanceof CarFactory) {
      System.out.println("2. 파트너 공장[" + ((CarFactory) partner).getFactoryName() + "] 협력 시 추가 생산량 = "
          + ((CarFactory) partner).workTogether(partner));
    } else if (partner instanceof TVFactory) {
      System.out.println("2. 파트너 공장[" + ((TVFactory) partner).getFactoryName() + "] 협력 시 추가 생산량 = "
          + ((TVFactory) partner).workTogether(partner));
    }
